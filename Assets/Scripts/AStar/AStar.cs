		...
		while (openList.Count > 0)//Step 10
		{
			...
			for (int x = -1; x <= 1; x++)
			{
				for (int y = -1; y <= 1; y++)
				{
					...
					if (LevelManager.Instance.InBounds(neighbourPos) && LevelManager.Instance.Tiles[neighbourPos].Walkable || LevelManager.Instance.InBounds(neighbourPos) && LevelManager.Instance.Tiles[neighbourPos] == LevelManager.Instance.Tiles[goal])
					{
