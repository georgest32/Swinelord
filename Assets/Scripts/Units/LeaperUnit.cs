...
public class LeaperUnit : Monster {
	...
	protected override void OnTriggerEnter2D(Collider2D other)
	{
		if (GameManager.Instance.WallToLeap != null) {
			...
			if (other.name == "WallUnitStopper" && other.transform.parent.parent.GetComponent<TileScript> ().GridPosition == GameManager.Instance.WallToLeap.GridPosition) {
				this.Speed = 0;
				isLeaping = true;

				DetermineLeapDestination ();
			}
		}
	...
	public void OnTriggerExit2D(Collider2D other)
	{
		if(!Leaped)
		{
		if (other.gameObject.tag == "Wall" && other.transform.parent.GetComponent<TileScript> ().GridPosition == GameManager.Instance.WallToLeap.GridPosition && !this.StoleBacon) 
			{
			...
			else if (other.gameObject.tag == "Wall" && other.transform.parent.GetComponent<TileScript> ().GridPosition == GameManager.Instance.WallToLeap.GridPosition && this.StoleBacon) 
			{
				this.Speed = this.MaxSpeed;
				isLeaping = false;
